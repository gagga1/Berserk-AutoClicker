...
use crate::engine::VariationCurve;
...
pub fn build_config(settings: &ClickerSettings) -> Result<ClickerConfig, String> {
    ...
    Ok(ClickerConfig {
        ...
        variation_curve: VariationCurve::from_str(&settings.speed_variation_curve),
        ...
        foreground_lock_enabled: settings.foreground_lock_enabled,
        foreground_lock_title: settings.foreground_lock_title.clone(),
    })
}
...
pub fn start_clicker(config: ClickerConfig, control: RunControl) -> RunOutcome {
    ...
    while control.is_active() {
        ...
        let batch_duration = if config.variation > 0.0 {
            let amount = cycle_duration_base * (config.variation / 100.0);
            match config.variation_curve {
                VariationCurve::Gaussian => {
                    rng.next_gaussian(cycle_duration_base, amount)
                }
                VariationCurve::Uniform => {
                    let r = rng.next_f64() * 2.0 - 1.0;
                    cycle_duration_base + amount * r
                }
                VariationCurve::Sine => {
                    let elapsed = start_time.elapsed().as_secs_f64();
                    let phase = (elapsed * 0.5).sin();
                    cycle_duration_base + amount * phase
                }
                VariationCurve::Walk => {
                    let drift = (rng.next_f64() * 2.0 - 1.0) * amount * 0.4;
                    walk_offset += drift;
                    walk_offset = walk_offset.clamp(-amount, amount);
                    cycle_duration_base + walk_offset
                }
            }
        } else {
            cycle_duration_base
        ...
        // Foreground-window gate: skip click batch if target window not focused
        if config.foreground_lock_enabled && !config.foreground_lock_title.is_empty() {
            if !crate::engine::mouse::foreground_window_matches(&config.foreground_lock_title) {
...
mod tests {
    ...
    fn sample_config() -> ClickerConfig {
        ClickerConfig {
            interval_secs: 0.04,
            variation: 0.0,
            variation_curve: VariationCurve::Gaussian,
            limit: 0,
            duty: 45.0,
            time_limit: 0.0,
            button: 1,
            ...
            foreground_lock_title: String::new(),
        }
    }
