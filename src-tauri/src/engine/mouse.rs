...
use windows_sys::Win32::UI::WindowsAndMessaging::{
    GetForegroundWindow, GetSystemMetrics, GetWindowTextLengthW, GetWindowTextW, SetCursorPos,
...
pub fn foreground_window_matches(needle: &str) -> bool {
    let needle = needle.trim().to_ascii_lowercase();
    if needle.is_empty() {
        return true;
    }
    match foreground_window_title() {
        Some(title) => title.to_ascii_lowercase().contains(&needle),
        None => false,
    }
}
