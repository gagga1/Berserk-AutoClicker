...
- `BERSERK_WINDOWS_SIGNING_MODE=trusted-signing-cli`
- `BERSERK_TRUSTED_SIGNING_ENDPOINT`
- `BERSERK_TRUSTED_SIGNING_ACCOUNT`
- `BERSERK_TRUSTED_SIGNING_PROFILE`
- `BERSERK_TRUSTED_SIGNING_DESCRIPTION` optional, defaults to `BerserkAutoClicker`
- `AZURE_CLIENT_ID`
- `AZURE_CLIENT_SECRET`
- `AZURE_TENANT_ID`
...
If `BERSERK_WINDOWS_SIGNING_MODE` is unset or set to `none`, the wrapper exits successfully without signing so the build can still complete.
...
```powershell
Get-AuthenticodeSignature src-tauri\target\release\bundle\nsis\BerserkAutoClicker_3.4.0_x64-setup.exe
```

Check the built executable:

```powershell
Get-AuthenticodeSignature src-tauri\target\release\BerserkAutoClicker.exe
```
