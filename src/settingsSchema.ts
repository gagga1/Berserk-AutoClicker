...
export type ThemePreset = "red" | "black" | "gold";
export type SpeedVariationCurve = "gaussian" | "uniform" | "sine" | "walk";
...
export interface PresetSnapshot {
  ...
  speedVariationCurve: SpeedVariationCurve;
...
export interface WindowProfileRule {
  id: string;
  titlePattern: string;
  presetId: PresetId;
}
...
export interface Settings extends PresetSnapshot {
  ...
  foregroundLockEnabled: boolean;
  foregroundLockTitle: string;
  hudEnabled: boolean;
  ...
  windowProfiles: WindowProfileRule[];
}
...
function sanitizeThemePreset(value: unknown): ThemePreset {
  if (value === "black" || value === "gold" || value === "red") {
    return value;
  }
  return "red";
}

function sanitizeSpeedVariationCurve(value: unknown): SpeedVariationCurve {
  if (
    value === "uniform" ||
    value === "sine" ||
    value === "walk" ||
...
export function buildPresetSnapshot(settings: Settings): PresetSnapshot {
  return {
    ...
    speedVariationCurve: settings.speedVariationCurve,
...
function sanitizePresetSnapshot(
  ...
  return {
    ...
    speedVariationCurve: sanitizeSpeedVariationCurve(saved.speedVariationCurve),
...
export function sanitizeSettings(
  ...
  return {
    ...
    foregroundLockEnabled: sanitizeBoolean(saved.foregroundLockEnabled, defaults.foregroundLockEnabled),
    foregroundLockTitle:
      typeof saved.foregroundLockTitle === "string"
        ? saved.foregroundLockTitle.slice(0, 200)
        : defaults.foregroundLockTitle,
    hudEnabled: sanitizeBoolean(saved.hudEnabled, defaults.hudEnabled),
    soundEnabled: sanitizeBoolean(saved.soundEnabled, defaults.soundEnabled),
    disableScreenshots: false,
    lastPanel: sanitizeSavedPanel(saved.lastPanel),
    theme: sanitizeTheme(saved.theme),
    themePreset: sanitizeThemePreset(saved.themePreset),
    ...
    activePresetId:
      ...
      presets.some((preset) => preset.id === saved.activePresetId)
        ...
        : null,
    windowProfiles: sanitizeWindowProfiles(saved.windowProfiles, presets),
  };
}

function sanitizeWindowProfiles(
  input: unknown,
  presets: PresetDefinition[],
): WindowProfileRule[] {
  ...
  return input
    .map((raw): WindowProfileRule | null => {
      if (typeof raw !== "object" || raw === null) return null;
      const r = raw as Partial<WindowProfileRule>;
      const titlePattern =
        typeof r.titlePattern === "string"
          ? r.titlePattern.slice(0, 200).trim()
          : "";
      ...
      if (!titlePattern || !presetIds.has(presetId)) return null;
      return { id, titlePattern, presetId };
    })
    .filter((r): r is WindowProfileRule => r !== null)
    .slice(0, 30);
}
